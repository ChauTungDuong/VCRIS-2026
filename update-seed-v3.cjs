...
// 1. Update helpers
const oldSection = `function ai4crisSection(opts: any = {}): PuckNode {
  return {
    type: "Ai4CrisSection",
    props: {
...
code = code.replace(oldSection, newHelpers);
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  // Wipe DB first to prevent duplicates
  ...
  // If the \n\n string format changed slightly
  const endStr = 'for (const pageData of pages) {';
  const endIdx2 = code.indexOf(endStr);
  if (startIdx !== -1 && endIdx2 !== -1) {
    code = code.substring(0, startIdx) + pagesArrayCode + '\n\n  ' + code.substring(endIdx2);
  ...
  for (const pageData of pages) {`;
    ...
    fs.writeFileSync('server/prisma/seed-ai4cris.ts', code);
    console.log("Successfully generated seed file (fallback method)");
  } else {
